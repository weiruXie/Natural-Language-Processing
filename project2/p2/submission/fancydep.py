...
class perceptron():
    ...
    def train(self, sentence):
        while sentence.buffer:
            ...
            #  update perceptron
            self.update(sentence.stack[-1], bufferTopWord, sentence.buffer, t_true)
    ...
    def predict(self, sentence):
        while sentence.buffer:
            ...
            t = self.predictTransition(sentence.stack[-1], bufferTopWord, sentence.buffer)
    ...
    def update(self, lWord, rWord, buff, truth):
        pred = self.predictTransition(lWord, rWord, buff)
        self.counter += 1
        self.accuracy[1] += 1
        if pred != truth:
            feats = self.getFeatures(lWord, rWord, buff)
    ...
    def predictTransition(self, lWord, rWord, buff):
        feats = self.getFeatures(lWord, rWord, buff)
    ...
    def getFeatures(self, lWord, rWord, buff):
        feats = {}
        ...
        feats["POS_pair:" + lWord.get("coarsePOS") + "_" + rWord.get("coarsePOS")+"_"+str(int(rWord.get("index")) - int(lWord.get("index")))] = 1.
        # feats["dis:" + str(int(rWord.get("index")) - int(lWord.get("index")))] = 1.
        # feats["word_pair2:" + rWord.get("word") + "_" + rWord2.get("word", "")] = 1.
        count = 0
        maxCount = 1
        for word in buff:
            count += 1
            if count > maxCount:
                break
            feats["rPOS_pair2:" + rWord.get("coarsePOS") + "_" + word.get("coarsePOS", "") + "_" +\
                    str(int(word.get("index")) - int(rWord.get("index")))] = 1.
            feats["lPOS_pair2:" + lWord.get("coarsePOS") + "_" + word.get("coarsePOS", "") + "_" +\
                    str(int(word.get("index")) - int(lWord.get("index")))] = 1.   
        return feats
