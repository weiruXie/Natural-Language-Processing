...
def computeGraphEdgeWeights(graph, weights):
    for i,j in graph.edges_iter():
        ...
        # TODO: your code here
        graph[i][j]['weight'] = math.exp(weights.dotProduct(graph[i][j]))
...
def perceptronUpdate(weights, G, true, pred):
    ...
    for i,j in pred.edges_iter():
        # TODO: your code here
        if not (i, j) in true.edges():
            f = true.node[i]
            g = true.node[j]
            weights.update({ 'w_pair=' + f['word'] + '_' + g['word']: 1,
                      'p_pair=' + f['pos' ] + '_' + g['pos' ]: 1,
                      'dist=' + str(abs(i-j)): 1 }, -1)
